 ...
 * 现在这些类被弃用了， 取而代之的是 AnayList 和 HashMap 类。这些类不是线程安全的，而集合库中提供了不同的机制。
 ...
 * while (iter.hasNext()) . .
 * }
 * 如果使用“ foreach” 循环,必须使用同样的代码， 因为循环使用了迭代器。
 * 注意：如果在迭代过程中，别的线程修改集合，迭代器会失效， 抛出 ConcurrentModificationException 异常。

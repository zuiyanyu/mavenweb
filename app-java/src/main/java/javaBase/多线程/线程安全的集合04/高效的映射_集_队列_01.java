 ...
 * 例如，   一个线程可能要开始向表中插入一个新元素。假定在调整散列表各个桶之间的链接关系的过程中， 被剥夺了控制权。如果另一个线程也开始
 *          遍历同一个链表， 可能使用无效的链接并造成混乱， 会抛出异常或者陷入死循环
 ...
 *
 * TODO 7.并发的散列映射表， 可高效地支持大量的读和一定数量的写。
 * 默认情况下， 假定可以有多达 16 个写线程同时执行。
 * 可以有更多的写线程，但是， 如果同一时间多于 16个，其他线程将暂时被阻塞。可以指定更大数目的构造器，然而， 恐怕没有这种必要。
 */
public class 高效的映射_集_队列_01 {
     ...
     *          loadFactor 控制调整如:果每一个桶的平均负载超过这个因子，表的大小会被重新调整。默认值为 0.75。

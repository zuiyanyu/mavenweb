package com.quartz.originCode.factory;
...
import com.quartz.originCode.property.PropertiesParser;
import org.quartz.*;
 ...
 */
public class StdSchedulerFactory_quartz implements SchedulerFactory {
     ...
     */
    public StdSchedulerFactory_quartz() {
    }
     ...
     */
    public StdSchedulerFactory_quartz(Properties props) throws SchedulerException {
        initialize(props);
    }
     ...
     */
    public StdSchedulerFactory_quartz(String fileName) throws SchedulerException {
        initialize(fileName);
    }
    ...
    private Scheduler instantiate() throws SchedulerException {
        ...
        JobStore js = null;        //用来提供一个job任务
        ThreadPool tp = null;      //线程池

        //QuartzScheduler是Quartz的核心调度器，是 org.quartz.Scheduler的间接实现，包含了所有调度org.quartz.Job和JobListener的方法
        //会加载\org\quartz\core\quartz-build.properties文件进行创建QuartzScheduler
        ...
        boolean wrapJobInTx = false;//是否将job进行包装，运行在事务环境中
        boolean autoId = false;
        long idleWaitTime = -1;//空闲等待时间
        long dbFailureRetry = 15000L; // 15 secs   失败重试
        ...
        //PROP_JOB_STORE_PREFIX = "org.quartz.jobStore"  ; PROP_JOB_STORE_LOCK_HANDLER_PREFIX= org.quartz.jobStore.lockHandler
        ...
        for (int i = 0; i < dsNames.length; i++) {
            ...
            if(cpClass != null) {
                ...
                //PROP_DATASOURCE_JNDI_URL = "jndiURL"
                ...
                if (dsJndi != null) {
                    ...
                    //这里取出来这三个配置属性，是因为要进行判断非空和决策用
                    //POOLING_PROVIDER = "provider"
                    ...
                    try {
                        //为当前的数据源提供Connection连接的实例对象。
                        ConnectionProvider cp = null;
                        try {
                            Constructor constructor = loadHelper.loadClass(cpClass).getConstructor(Properties.class);
                            //使用配置文件中的 org.quartz.dataSource.数据源名称.driver 等配置进行实例化ConnectionProvider
                            //这个构造器使用的是 com.mchange.v2.c3p0.ComboPooledDataSource 数据源
                        ...
                        //DBConnectionManager 是一个单例模式，管理不同的数据源
                        dbMgr = DBConnectionManager.getInstance();
                        //将这个数据源交给DBConnectionManager进行管理：key = 数据源名称， value= ConnectionProvider实例
                        dbMgr.addConnectionProvider(dsNames[i], cp);
    ...
    public Scheduler getScheduler() throws SchedulerException {
        ...
        //2.2获取调度器存储仓库，如果没有被初始化，就进行初始化。
        //调度器存储仓库的作用是：保存对调度器实例的引用——确保惟一性防止垃圾收集，并允许'全局'查找-因为所有调度器都在当前类中。
        SchedulerRepository schedRep = SchedulerRepository.getInstance();
        //2.3  根据到调度器的名称，从调度器存储仓库中获取已经存在的调度器，如果不存在，就返回null.
        //调度器名称是默认为"QuartzScheduler"，可以在配置文件中进行配置：
        //org.quartz.scheduler.instanceName: DefaultQuartzScheduler
        Scheduler sched = schedRep.lookup(getSchedulerName());

        //2.4 如果从调取器仓库获取到了一个调度器，并且可用，就进行返回,不可用就进行移除。
        if (sched != null) {
            if (sched.isShutdown()) {
                //如果获取到的调度器已经关闭，那么就从调度器仓库中移除此调度器。
        ...
        //  2.5 如果从调度器仓库没有获取到一个调度器，或者获取到的调度器已关闭，调用instantiate()方法。
        //  1） 获取SchedulerRespoitory实例。
        // 2）创建一个Scheduler实例 。
        // 3）调用SchedulerRespoitory的bind(Scheduler scheduler)，将Scheduler实例保存到任务调器仓库内。
    ...
    public static Scheduler getDefaultScheduler() throws SchedulerException {
        org.quartz.impl.StdSchedulerFactory fact = new org.quartz.impl.StdSchedulerFactory();
